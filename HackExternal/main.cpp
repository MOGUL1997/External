	...
	while (true)
	{
		...
		if (isAimbotActive)
		{
			...
			for (int index = 0; index < EntityList_Copy.size(); ++index)
			{
				...
				auto neck_01_pos = GetBoneWithRotation(Entity.actor_mesh, bones::neck_01);
				auto targetneck_01 = ProjectWorldToScreen(Vector3(neck_01_pos.x, neck_01_pos.y, neck_01_pos.z + 15));

				float x = targetneck_01.x - GameVars.ScreenWidth / 2.0f;
				float y = targetneck_01.y - GameVars.ScreenHeight / 2.0f;
			...
			if (target_entity.actor_mesh != 0 || target_entity.actor_pawn != 0 || target_entity.actor_id != 0)
			{
				...
				auto neck_01_pos = GetBoneWithRotation(target_entity.actor_mesh, bones::neck_01);
				auto targetneck_01 = ProjectWorldToScreen(Vector3(neck_01_pos.x, neck_01_pos.y, neck_01_pos.z + 15));
				move_to(targetneck_01.x, targetneck_01.y);
			}
		}
		Sleep(10);
	}
...
void Render()
{
	...
	// Change values for "flaming red" color
	style->Colors[ImGuiCol_BorderShadow] = ImVec4(1.00f, 0.00f, 0.00f, 1.00f);
	style->Colors[ImGuiCol_FrameBg] = ImVec4(1.00f, 0.00f, 0.00f, 1.00f);
	style->Colors[ImGuiCol_FrameBgHovered] = ImVec4(1.00f, 0.00f, 0.00f, 1.00f);
	...
	style->Colors[ImGuiCol_CheckMark] = ImVec4(0.00f, 0.00f, 0.00f, 1.00f); // Black check mark
	style->Colors[ImGuiCol_ScrollbarBg] = ImVec4(0.00f, 0.00f, 0.00f, 1.00f); // Red background
	style->Colors[ImGuiCol_ScrollbarGrab] = ImVec4(0.00f, 0.00f, 0.00f, 1.00f); // Black grabber
	style->Colors[ImGuiCol_ScrollbarGrabHovered] = ImVec4(0.56f, 0.56f, 0.58f, 1.00f); // Hovered light gray grabber
	style->Colors[ImGuiCol_ScrollbarGrabActive] = ImVec4(1.00f, 0.00f, 0.00f, 1.00f); // Active light gray grabber
	...
	style->Colors[ImGuiCol_Header] = ImVec4(1.00f, 0.00f, 0.00f, 1.00f);
	style->Colors[ImGuiCol_HeaderHovered] = ImVec4(1.00f, 0.00f, 0.00f, 1.00f);
	style->Colors[ImGuiCol_HeaderActive] = ImVec4(1.00f, 0.00f, 0.00f, 1.00f);

	// Leave the rest of the colors unchanged
	...
	{
		InputHandler();
		ImGui::SetNextWindowSize(ImVec2(426, 170)); // 450,426
		ImGui::PushFont(DefaultFont);
		ImGui::Begin("MOGULCYRE TESTING (AutoNeck)", 0, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar);
		
		TabButton("Aim Assist", &CFG.tab_index, 0, true);

		ImGui::Separator();

		if (CFG.tab_index == 0)
		{
			ImGui::Checkbox("Enable AimAssist", &CFG.b_Aimbot);		
				ImGui::Combo("Assist Key", &CFG.AimKey, keyItems, IM_ARRAYSIZE(keyItems));
